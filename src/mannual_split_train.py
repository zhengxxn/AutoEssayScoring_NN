...
import sys
import yaml
from util.convenient_funcs import create_path, print_model, get_path_prefix, get_feature_from_ids
...
def main():
    ...
    essay_set = set(dataset['essay_set'])
    ...
    for set_id, essay_set_id in enumerate(essay_set):
        ...
        dataset_in_set = dataset[dataset.essay_set == essay_set_id]
        dataset_essays = dataset_in_set.essay.values
        dataset_labels = dataset_in_set.domain1_score.values
        dataset_ids = dataset_in_set.essay_id.values

        train_essays, validation_essays, train_labels, validation_labels = train_test_split(dataset_essays,
                                                                                            dataset_labels,
                                                                                            random_state=0,
                                                                                            test_size=config['dev_dataset_ratio'])
        _, _, train_ids, validation_ids = train_test_split(dataset_labels,
                                                           dataset_ids,
                                                           random_state=0,
                                                           test_size=config['dev_dataset_ratio'])
        ...
        for current_epoch in trange(epoch, desc='Epoch'):
            ...
            with torch.no_grad():
                for batch in validation_dataloader:
                    ...
                    dev_loss.append(result['loss'].item() / batch_inputs.shape[0])
